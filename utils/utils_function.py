...
def entire_preprocessing(x, y, threshold=0.9):
    ...
    print('Deleted by standard deviation: {}'.format(len(features_delete_std)))

    features_delete_R = delete_similarity(x, y, threshold)
    x = x.drop(features_delete_R, axis=1)
    print('Deleted by similarity: {}'.format(len(features_delete_R)))

    # Z-score
    data_preprocessed = x.apply(lambda x: (x - np.mean(x)) / (np.std(x)))

    print('Feature map shape: {}'.format(data_preprocessed.shape))
